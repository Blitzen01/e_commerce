...
// Query for the first table, excluding 'Declined' status
$weekly_transaction_history = "SELECT * FROM transaction_history WHERE DATE(transaction_date) BETWEEN '$startOfWeek' AND '$endOfWeek' AND status != 'Declined'";
$weekly_transaction_history_result = mysqli_query($conn, $weekly_transaction_history);

// Query for the second table, excluding 'Declined' status
$weekly_order_transaction_history = "SELECT * FROM order_transaction_history WHERE DATE(transaction_date) BETWEEN '$startOfWeek' AND '$endOfWeek' AND status != 'Declined'";
...
// Query for the first table, excluding 'Declined' status
$monthly_pending_booking = "SELECT * FROM transaction_history WHERE MONTH(transaction_date) = '$currentMonth' AND status != 'Declined'";
$monthly_pending_booking_result = mysqli_query($conn, $monthly_pending_booking);

// Query for the second table, excluding 'Declined' status
$monthly_pending_order = "SELECT * FROM order_transaction_history WHERE MONTH(transaction_date) = '$currentMonth' AND status != 'Declined'";
...
// Query for the first table, excluding 'Declined' status
$yearly_booking_monitoring = "SELECT * FROM transaction_history WHERE YEAR(transaction_date) = '$currentYear' AND status != 'Declined'";
$yearly_booking_monitoring_result = mysqli_query($conn, $yearly_booking_monitoring);

// Query for the second table, excluding 'Declined' status
$yearly_order_monitoring = "SELECT * FROM order_transaction_history WHERE YEAR(transaction_date) = '$currentYear' AND status != 'Declined'";
